...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* panel */}
      <aside
        className={`fixed right-0 top-0 z-[70] h-full w-[92vw] max-w-[420px] bg-white shadow-2xl transition-transform duration-300 ${opened ? 'translate-x-0' : 'translate-x-full'}`}
        role="dialog"
        aria-modal="true"
                ...
                <div key={`${i.slug}-${i.size}`} className="flex gap-3 border rounded-2xl p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 inline-flex items-center rounded-xl border">
                      <button className="w-8 h-8" onClick={() => setQty(i.slug, i.size, i.qty - 1)} aria-label="Decrease">−</button>
                      <span className="w-10 text-center">{i.qty}</span>
                      <button className="w-8 h-8" onClick={() => setQty(i.slug, i.size, i.qty + 1)} aria-label="Increase">+</button>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm">${(i.price * i.qty).toFixed(2)}</p>
                    <button onClick={() => removeItem(i.slug, i.size)} className="text-[11px] text-red-600 mt-2">
                      Remove
                    </button>
                  </div>
                </div>
